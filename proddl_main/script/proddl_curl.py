...
class MakeflowWriter(object):
    ...
    def createJobList(self, iterCount, timerId, masterAddr, inputId):
        ...
        default_sub_depend = ["$PYTHON","timer.py","$CURL","$TAR"]
        for cnt in range(iterCount):
            job = Task()
            job.jobId=cnt
            job.depend.extend(default_sub_depend)
            job.outputs.extend([str(cnt)+".log"])
            job.scriptName = _worker_cmd_tpl.format(ind=cnt, masterAddr=masterAddr, inputId=inputId)
            jobs.append(job)        
                ...
                if 'file' in job:
                    ...
                    if args[2]=='path':
                        if idx==0: 
                            ...
                            writer.createJobList(50, timerFile, data['host'], inputFile) #makeflow job iteration count
                            writer.close()
...
class taskData:
    def __init__(self, jobid, data):
        self.startTime = self.massageTime(data[6])
        tMap={
            'jobid':jobid,
            'taskid':data[2],
            't_stamp':self.massageTime(data[1]),
            'result':int(data[3]),
            'return':int(data[4]),
            'algo':data[5],
